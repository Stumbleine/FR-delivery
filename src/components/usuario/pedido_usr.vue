    ...
    <v-container>
            ...
            <template v-slot:default>
                <thead tile class="indigo">
                    <tr class="text-uppercase">
                        <th class="text-center" style="font-size:15px;">Nombre producto</th>
                        <th class="text-center" style="font-size:15px;">Cantidad</th>
                        <th class="text-center" style="font-size:15px;">Precio unidad</th>
                        <th class="text-center" style="font-size:15px;">Subtotal</th>
                        <th class="text-center" style="font-size:15px;"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(item,index) in lista" :key="item.id" class="text-center">
                        <td>{{ item.nombre }}</td>
                        <td>
                            {{ item.cantidad }}
                            <div class="ml-5 d-inline-flex">
                                ...
                                <v-btn x-small @click="disminuir(index),calcularTotal()">
                        ...
                        <td>{{item.subtotal}}</td>
                        <td>
                            <v-btn fab x-small @click="deletePro(index),calcularTotal()">
        ...
        <v-form>
            <v-container>
                <v-row>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="pedido.nombre_cliente" label="Nombre de referencia" required></v-text-field>
                    </v-col>

                    <v-col cols="12" md="6">
                        <v-text-field v-model="pedido.numero_cliente" label="Telefono o celular" required></v-text-field>
                    </v-col>
                </v-row>
            </v-container>
            <p>{{pedido}}</p>
        <div class="d-flex justify-center mt-8" width="100%" style="margin-bottom: 10em;">
            <v-btn color="primary" class="d-flex" height="50" @click="dates(),guardarPedido()">Realizar Pedido</v-btn>
        </div>
        </v-form>
...
export default {
    ...
    methods:{
        ...
        guardarPedido(){
            ...
            console.log(envio)
            axios.post("http://localhost:8080/api/pedido", envio,{ headers: {'Content-Type': 'application/json'}} ).then(()=>{console.log(this.pedido);}
